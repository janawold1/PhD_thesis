...
pixy_pi <- read.table("pixy/100kb_window/Pruned_MAF/total/total_MAF_100kb_window_pi.txt", header = TRUE, sep = "\t")
pixy_dxy <- read.table("pixy/100kb_window/Pruned_MAF/total/total_MAF_100kb_window_dxy.txt", header = TRUE, sep = "\t")

nuc_compare <- cbind(pixy_pi, pixy_dxy[c("avg_dxy")])

#png("plots/nucleotide_diversity_relationship_AU.png")
ggplot(subset(nuc_compare, pop %in% c("AU")), aes(x = avg_dxy, y = avg_pi)) +
  geom_point() +
  labs(title = "Austrlian fairy tern pi and DXY") +
  theme_light()
#dev.off()

#png("plots/nucleotide_diversity_relationship_TI.png")
ggplot(subset(nuc_compare, pop %in% c("TI")), aes(x = avg_dxy, y = avg_pi)) +
  geom_point() +
  labs(title = "Tara iti pi and DXY")
  theme_light()
#dev.off()
  
ggplot(nuc_compare, aes(x = pop, y= avg_pi)) +
  geom_violin() +
  theme_light()
ggplot(nuc_compare, aes(x = pop, y= avg_dxy)) +
  geom_violin() +
  theme_light()
